...
describe CensusReminderJob do
  ...
  let(:leaders) do
    all = []
    all << Fabricate(Group::Flock::Leader.name.to_sym, group: flock,
      person: Fabricate(:person, email: "test1@example.com")).person
    all << Fabricate(Group::Flock::Leader.name.to_sym, group: flock,
      person: Fabricate(:person, email: "test2@example.com")).person
