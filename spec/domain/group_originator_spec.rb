...
describe GroupOriginator do
  ...
  context "flock" do
    it "deleted are considered" do
      create([Group::Flock::Leader, groups(:bern), created_at: 2.days.ago, end_on: 1.day.ago])
    ...
    it "active childgroup overrides deleted flock" do
      create([Group::Flock::Leader, groups(:innerroden), created_at: 2.weeks.ago, end_on: 5.days.ago],
        [Group::ChildGroup::Child, groups(:asterix)])
      assert_originating_groups flock: groups(:bern), state: groups(:be)
    end
  ...
  context "without flock" do
    it "deleted are considered" do
      create([Group::StateBoard::Leader, groups(:be_board), created_at: 2.days.ago, end_on: 1.day.ago])
