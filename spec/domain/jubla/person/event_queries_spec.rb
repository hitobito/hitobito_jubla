...
describe Person::EventQueries do
  ...
  context :coach do
    ...
    it "can see event up to one month" do
      event1.dates.first.update!(finish_at: 20.days.from_now)
      event2.dates.first.update!(finish_at: 28.day.ago)
      expect(subject.count).to eq(2)
      expect(queries.upcoming_events).to eq []
    end

    it "can not see event more than one month ago" do
      event1.dates.first.update!(finish_at: 32.day.ago)
      event2.dates.first.update!(finish_at: 2.years.ago)
      expect(subject.count).to eq(0)
    end

    it "ordered by date" do
      event1.dates.first.update_attribute(:start_at, 2.days.from_now)
      event2.dates.first.update_attribute(:start_at, 23.day.ago)
  ...
  context :advisor do
    ...
    it "can see event 20 days ago" do
      event1.dates.first.update!(finish_at: 20.day.ago)
      expect(subject.count).to eq(1)
      expect(queries.upcoming_events).to eq []
    end

    it "can not see event 40 days ago" do
      event1.dates.first.update!(finish_at: 40.day.ago)
      expect(subject.count).to eq(0)
    end
  end
