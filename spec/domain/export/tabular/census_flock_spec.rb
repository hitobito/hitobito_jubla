...
describe Export::Tabular::CensusFlock do
  ...
  describe ".headers" do
    ...
    its(:labels) do
      should eq ["Kanton", "Region", "Art", "Schar", "Kontakt Vorname", "Kontakt Nachname", "Adresse", "PLZ", "Ort",
  ...
  describe "census flock" do
    ...
    it "ignores archived group" do
      ...
      expect(census_flock.list).to have(3).items
    end
  end
  ...
  describe "mapped items" do
    ...
    describe "keys and values" do
      ...
      its(:keys) do
        should eq [:state, :region, :kind, :name, :contact_first_name, :contact_last_name, :address, :zip_code, :town,
                   :jubla_property_insurance, :jubla_liability_insurance, :jubla_full_coverage,
                   :leader_count, :child_count]
      end

      its(:values) { should eq ["Kanton Bern", "Stadt", "Jungwacht", "Bern", nil, nil, nil, nil, nil, false, false, false, 5, 7] }
    ...
    describe "address, zip code and town" do
      ...
      its(:values) do
        should eq ["Kanton Bern", "Stadt", "Jungwacht","Bern", nil, nil, "bar 23", 1234, "foo", false, false, false, 5, 7]
      end
    end
    ...
    describe "contact person" do
      ...
      its(:values) { should eq ["Kanton Bern", "Stadt", "Jungwacht","Bern", "Top", "Leader", nil, nil, nil, false, false, false, 5, 7] }
    end
    ...
    describe "insurance attributes" do
      ...
      its(:values) { should eq ["Kanton Bern", "Stadt", "Jungwacht","Bern", nil, nil, nil, nil, nil, true, true, true, 5, 7] }
    end

    describe "without member count" do
      before { MemberCount.where(flock_id: flock.id).destroy_all }

      its(:values) { should eq ["Kanton Bern", "Stadt", "Jungwacht","Bern", nil, nil, nil, nil, nil, false, false, false, nil, nil] }
    end
  end
