...
describe EventAbility do
  ...
  let(:participation) {
    Fabricate(:event_participation, participant: participant, event: event,
      application: Fabricate(:jubla_event_application))
  }
  ...
  context :participations_read do
    ...
    before { Fabricate(Event::Role::Speaker.name.to_sym, participation: participation) }
