...
describe GroupsController, type: :controller do
  ...
  describe_action :delete, :destroy do
    [[:leader, :asterix, true],
      [:agent, :asterix, true]].each do |user, group, can_destroy_group|
    ...
    [[:agent, :flock, false],
      [:agent, :region, false]].each do |user, group, can_destroy_group|
      context "#{user} destroy #{group}" do
        before { sign_in(send(user)) }
    ...
    [[:leader, :flock, false],
      [:agent, :state, false]].each do |user, group, can_destroy_group|
      context "#{user} destroy #{group}" do
        before { sign_in(send(user)) }
